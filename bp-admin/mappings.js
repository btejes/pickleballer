(function(){
  ...
  let listEl = document.getElementById('bp-mapping-list');
  let saveBtn = document.getElementById('bp-mapping-save-btn');
  ...
  function renderList() {
    let categories = window.bpAdminCategories
    ...
    let questions = (window.BP_QUESTIONS || []).filter(function(q){
    ...
    questions.forEach(function(q){
      ...
      let hRight = document.createElement('span');
      ...
      q.answers.forEach(function(a){
        ...
        let catsWrap = document.createElement('div');
        catsWrap.className = 'flex flex-wrap gap-1.5';

        let currentCats = (mappings[q.id] && mappings[q.id][a.id]) || [];
        ...
        categories.forEach(function(catName){
          ...
          let isOn = currentCats.indexOf(catName) !== -1;
          if (isOn) {
