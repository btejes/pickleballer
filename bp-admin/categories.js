import { api, showToast, withSpinner, spinnerSvg, onInit, onTab } from './app.js';

const BASE_CATEGORIES = ['Power', 'Control', 'Spin'];
function isBase(name) {
  const n = String(name || '').trim().toLowerCase();
  return BASE_CATEGORIES.some(function(b){ return b.toLowerCase() === n; });
}

const addBtn = document.getElementById('bp-category-add-btn');
const newInput = document.getElementById('bp-new-category');
const listEl = document.getElementById('bp-category-list');
const countEl = document.getElementById('bp-category-count');

let cached = [];

export function getCategories() { return cached.slice(); }
...
function loadCategories(silent) {
  ...
  return api('list_categories').then(function(res){
    ...
    renderList();
    window.dispatchEvent(new CustomEvent('bp-categories-changed'));
  });
}
...
function renderList() {
  ...
  const sorted = cached.slice().sort(function(a, b){
    const ab = isBase(a.name) ? 0 : 1;
    const bb = isBase(b.name) ? 0 : 1;
    if (ab !== bb) return ab - bb;
    return String(a.name).toLowerCase().localeCompare(String(b.name).toLowerCase());
  });
  sorted.forEach(function(cat){
    const base = isBase(cat.name);
...
onInit(function(){ loadCategories(); });
onTab('categories', function(){ loadCategories(); });
